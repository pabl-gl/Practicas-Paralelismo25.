...
int main(int argc, char *argv[]) {
    ...
    resto = Y_RESN % nprocs;
    if (rank < resto) {
        // los primeros procesos tendrán una fila más para cubrir el resto
    ...
    gettimeofday(&tci, NULL); //Fin tiempo computacion
    ...
    double all_tcomm[nprocs];
    double all_tcomp[nprocs];
    MPI_Gather(&tcomm, 1, MPI_DOUBLE, all_tcomm, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Gather(&tcomp, 1, MPI_DOUBLE, all_tcomp, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        if (DEBUG) {
            ...
            for (i = 0; i < nprocs; i++) {
                fprintf(stderr,"Proceso %d: Tiempo computacional = %.6lf s\tTiempo comunicación = %.6lf s\n", i, all_tcomp[i], all_tcomm[i]);
            }
            free(vres);
        }
    }
